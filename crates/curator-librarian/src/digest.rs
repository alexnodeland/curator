...
use curator_core::note::{Frontmatter, Note, NoteFrontmatter};
...
pub enum DigestError {
    ...
    #[error("no index at {0} — run `curator ingest` first")]
    IndexMissing(PathBuf),
}

/// What one digest run did — the `--json` summary of `curator digest run`.
...
///   identities); otherwise the proposal waits for `curator apply`.
///
...
pub fn run_digest(
    ...
    if auto {
        ...
        let (_, patch) = curator_core::load_proposal(&vault, proposals_dir, &proposal.id)
...
mod tests {
    use super::*;
    use curator_index::HashEmbedder;
