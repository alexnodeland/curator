...
use curator_index::{ChunkParams, HashEmbedder, Index, IndexError, IndexReader, build_epoch};
...
fn incremental_ingest_updates_in_place_same_epoch() {
    ...
    let note = curator_core::Note::parse("a.md", "rewritten amended wording\n").expect("parses");
...
/// it on `curator reindex` silently skips every tombstone that fired before
