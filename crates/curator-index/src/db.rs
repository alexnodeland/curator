...
//! CLI commands (`curator ingest`, `curator reindex`) — one process, one command,
...
//! `curator ingest`/`curator reindex`/`curator zotero sync` in a different process —
...
impl IndexReader {
    ...
    /// [`Index::cursors_for`] (`curator doctor` inspects tail health without a
    /// writer handle).
    pub fn cursors_for(&self, consumer: &str) -> Result<Vec<(String, i64)>, IndexError> {
        let mut stmt = self
            .conn
