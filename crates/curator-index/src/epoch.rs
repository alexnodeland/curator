...
use curator_core::{KpConfig, Vault};
...
/// heading-aware markdown chunker in curator-ingest, which this crate must not
...
/// The note corpus an epoch build indexes. Richer pipelines (curator-ingest:
...
pub fn build_epoch_from(
    ...
    // concurrent writer (curator ingest mid-run) must never have its WAL
    ...
    // log). Without this, a routine `curator reindex` would drop e.g. the
