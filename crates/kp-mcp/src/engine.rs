...
//! design — see kp-index's writer-discipline docs; per-session stdio
...
//! kp-core's `create_proposal` — there is no other write path.

use kp_core::{KpConfig, ProposalWriteError, Vault, VaultError};
use kp_index::embed::EmbedError;
use kp_index::{Embedder, IndexError, IndexReader, SearchHit, embedder_from_config};
...
pub const PROPOSAL_AUTHOR: &str = "kp-mcp";
...
impl KpEngine {
    ...
    /// surfaces per-operation, so `kp mcp serve` can start before the
    /// first epoch build.
    ...
    pub fn recent(
        ...
        let cutoff_secs = kp_core::time::unix_now().saturating_sub(u64::from(days) * 86_400);
        let cutoff = kp_core::time::rfc3339_utc(cutoff_secs);
        let reader = self.reader()?;
        let notes = reader
    ...
    /// written; `kp apply` does that after human review.
    pub fn propose(
        &self,
        title: &str,
        rationale: &str,
        ...
        let files: Vec<kp_core::ProposalFile> = files
            .iter()
            .map(|f| kp_core::ProposalFile {
                path: f.path.clone(),
