...
//! the vault working tree (rendered by kp-core via the `similar` crate).
...
//! change.
//!
//! The dialect parsed here is exactly what kp-core emits: `--- a/<path>`
...
pub enum PatchError {
    /// The patch text is not the dialect kp-core emits.
...
mod tests {
    use super::*;
    use kp_core::{ProposalFile, Vault, create_proposal};

    /// Render a patch exactly the way kp-core does, via a real proposal.
    fn kp_core_patch(existing: &[(&str, &str)], proposed: &[(&str, &str)]) -> String {
    ...
    fn round_trips_a_creation() {
        let patch = kp_core_patch(&[], &[("notes/new.md", "# Title\n\nbody line\n")]);
    ...
    fn round_trips_a_modification() {
        let old = "one\ntwo\nthree\nfour\nfive\n";
        let new = "one\n2\nthree\nfour\nfive\nsix\n";
        let patch = kp_core_patch(&[("n.md", old)], &[("n.md", new)]);
        let files = parse_patch(&patch).expect("parses");
    ...
    fn round_trips_missing_trailing_newlines() {
        ...
        let patch = kp_core_patch(&[("n.md", "one\n")], &[("n.md", "one\ntwo")]);
        let files = parse_patch(&patch).expect("parses");
        assert_eq!(
    ...
    fn drifted_content_is_non_clean() {
        let old = "one\ntwo\nthree\n";
        let patch = kp_core_patch(&[("n.md", old)], &[("n.md", "one\n2\nthree\n")]);
        let files = parse_patch(&patch).expect("parses");
