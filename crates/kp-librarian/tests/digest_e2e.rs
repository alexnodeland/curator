...
use kp_core::{KpConfig, KpId, ProposalStatus, Vault, list_proposals};
use kp_index::{HashEmbedder, IndexReader};
use kp_librarian::{is_uuid7, run_digest};
...
fn ingest_then_auto_digest_lands_the_note_and_reruns_are_noops() {
    ...
    let ingested = kp_ingest::ingest(&config, &e).expect("ingest");
...
fn identical_environments_produce_byte_identical_digests() {
    ...
    let render = || {
        let dir = tempfile::tempdir().expect("tempdir");
        let config = build_env(dir.path());
        kp_ingest::ingest(&config, &e).expect("ingest");
