//! `kp zotero sync` orchestration, end to end: wiremock API + temp vault +
...
use kp_index::{HashEmbedder, Index};
use kp_zotero::sync::{CURSOR_CONSUMER, CURSOR_FILE, SyncOptions, sync};
use kp_zotero::{MANAGED_END, ZoteroError};
...
impl Harness {
    ...
    fn config(&self, extra: &str) -> kp_core::KpConfig {
        let toml = format!(
            "schema = \"kp-config/v1\"\n\
             [vault]\npath = \"{}\"\n\
             [zotero]\nenabled = true\napi_base = \"{}\"\nuser_id = \"777\"\napi_key_env = \"{}\"\n{extra}",
        ...
        kp_core::KpConfig::from_toml_str(&toml).expect("config parses")
    }

    fn run(&mut self, extra_config: &str) -> Result<kp_zotero::SyncReport, ZoteroError> {
...
fn initial_sync_writes_notes_and_persists_the_cursor() {
    ...
    // The cursor persisted in kp-index.
    assert_eq!(
        h.index
...
fn disabled_configurations_return_clean_reports_without_network() {
    ...
    let config = kp_core::KpConfig::from_toml_str(&toml).expect("config");
