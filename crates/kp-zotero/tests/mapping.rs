...
//! kp-zotero --test mapping`, then review the diff — the goldens are the
//! mapping contract.

use std::path::PathBuf;

use kp_zotero::item::Item;
use kp_zotero::map::map_item;
...
fn golden_journal_article_with_attachment_and_fulltext() {
    ...
    let fulltext: kp_zotero::item::Fulltext =
...
fn rendered_notes_reingest_cleanly_as_kp_notes() {
    ...
    use kp_zotero::managed::split_managed;
    ...
    for fixture in [PAGE1, PAGE2] {
        for it in items(fixture).iter().filter(|i| !i.is_attachment()) {
            ...
            assert_eq!(
                ...
                &kp_core::Checksum::compute(split.managed.as_bytes()),
