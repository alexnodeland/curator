//! `curator zotero sync` — the two-channel orchestration.
//!
...
//! persist the library-version cursor in curator-index (consumer state, so
//! it survives `curator reindex`: epoch rebuilds carry cursors forward —
...
use curator_core::{KpConfig, Note, Vault};
use curator_index::Index;
...
/// contract's `[zotero]` table is implemented verbatim in `curator-core`;
...
/// What one sync run did — the `--json` summary of `curator zotero sync`.
