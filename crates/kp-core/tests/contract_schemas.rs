...
//! normative, so what kp-core WRITES must validate against them, and the
...
use kp_core::{KpId, Note, NoteFrontmatter, ProposalFile, Vault, create_proposal};
...
#[test]
fn kp_note_schema_accepts_what_kp_core_parses_and_serializes() {
    let schema = compile(KP_NOTE_SCHEMA);

    // The minimal block kp-core mints.
    let minimal =
    ...
    let kp_core::Frontmatter::Kp(fm) = &note.frontmatter else {
