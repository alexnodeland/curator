//! End-to-end librarian flows through the `kp` binary: ingest → digest
...
fn digest_run_auto_is_idempotent_by_date() {
    ...
    assert!(kp(&config, &["ingest"]).status.success());
    let out = ok_json(&kp(
        &config,
    ...
    let proposals = ok_json(&kp(&config, &["proposals", "list", "--json"]));
...
fn propose_review_apply_round_trip_and_reject_on_drift() {
    ...
    let review = kp(&config, &["review", &id]);
    ...
    let out = ok_json(&kp(&config, &["apply", &id, "--json"]));
...
fn doctor_reports_health_before_and_after_ingest() {
    ...
    assert!(kp(&config, &["ingest"]).status.success());
    let checks = ok_json(&kp(&config, &["doctor", "--json"]));
