...
use curator_core::{Checksum, KpConfig, Note, Vault};
use curator_index::{Chunk, ChunkParams, Embedder, EpochSource, Index, build_epoch_from};
...
/// What one ingest run did — the `--json` summary of `curator ingest`.
...
/// What one `curator index rebuild` did — the `--json` summary.
