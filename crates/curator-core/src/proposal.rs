...
pub struct Proposal {
    ...
    pub created: String,
    /// `curator-librarian` or an agent-supplied name.
    pub author: String,
...
/// Proposal lifecycle. Stamped by `curator apply` / `curator reject` — never edited
/// by agents.
...
/// themselves are NOT written — that is `curator apply`'s job, after human
/// review.
...
/// separately. Public: the apply-side validator (curator-librarian) enforces
...
/// oracle — curator-ingest — covers files this shape test cannot.)
#[must_use]
