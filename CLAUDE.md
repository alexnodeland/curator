...
| `crates/kp-core` | vault model, contract data types, identity, config loading, proposals validator |
| `crates/kp-index` | chunker, `Embedder` trait (`builtin` ONNX / `hash` test), one `index.db` (sqlite-vec + FTS5 + edges), blue/green epochs |
| `crates/kp-ingest` | producer adapters (Curio notes + events tail), ingest orchestration |
| `crates/kp-zotero` | read-only two-channel Zotero producer |
| `crates/kp-mcp` | the one MCP entrypoint — MCP surface v1 |
| `crates/kp-librarian` | deterministic zero-LLM digest baseline; LLM harness = optional prose enhancer |
| `crates/kp-cli` | the `kp` binary |
